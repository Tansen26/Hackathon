...
# ---------------------------
# FORECAST FUNCTION (Using Polynomial Regression)
# ---------------------------
...
ax2.fill_between(future_dates, predictions - std_dev, predictions + std_dev, 
...
# MODEL ACCURACY
# ---------------------------
# Calculate MAE on last 30 days
from sklearn.metrics import mean_absolute_error

# Create polynomial features for all data
df['date_numeric'] = (df['arrival_date'] - df['arrival_date'].min()).dt.days
...
predictions_all = model.predict(X_all_poly)

mae = mean_absolute_error(df['modal_price'].values[-30:], predictions_all[-30:])
mape = np.mean(np.abs((df['modal_price'].values[-30:] - predictions_all[-30:]) / df['modal_price'].values[-30:])) * 100
